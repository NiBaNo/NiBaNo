<div align="right">

🌎 Idioma: 
[English](README.md) | [Español](README.es.md) | [Català](README.ca.md)

</div>

<h1 align="center">Nil Bagaria</h1>

<h3 align="center">
Estudiant d’últim curs d’Enginyeria Informàtica · Enfocat en ciberseguretat
</h3>

<p align="center">
La Salle – Universitat Ramon Llull
</p>

<p align="center">
Experiència pràctica en entorns empresarials d’IT, xarxes i seguretat.
</p>

---

## 🔒 Experiència en Ciberseguretat

**BonPreu S.A.U. (pràctiques d’estiu)**
Experiència en equips d’IT, xarxes i ciberseguretat en un entorn empresarial real.

**Seguretat i Infraestructura**

* FortiGate (firewalls)
* FortiAuthenticator (VPN / autenticació)
* Trend Vision One (seguretat endpoint)
* Microsoft Azure (usuaris, logs, MFA)
* Cisco CES (seguretat de correu)
* Prisma Access (SASE - Palo Alto)
* Microsoft Purview

**Sistemes i Gestió de Dispositius**

* Active Directory
* KACE K1000
* Workspace ONE (gestió de dispositius, desplegaments, certificats)

**Altres eines**

* Jira
* 1Password
* Centrex (sistemes de telefonia)

---

## 🧠 Base Tècnica

Base acadèmica sòlida en:

* Sistemes Operatius
* Xarxes
* Sistemes de fitxers (FAT32, EXT)
* Sistemes encastats
* Estructures de dades i algorismes

Experiència programant en:

* C, Java, Python
* JavaScript, PHP, HTML
* SQL, Assembly, Swift 

---

## 🎯 Focus

* Ciberseguretat
* Funcionament intern dels sistemes
* Entorns reals d’infraestructura i seguretat

---

## 📌 Projectes

Els projectes estan disponibles als repositoris.

---

## 📫 Contacte

* Barcelona, Catalunya
* nilbagaria@gmail.com

---
